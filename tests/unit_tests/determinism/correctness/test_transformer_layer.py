...
1. ``test_bit_exact_under_parallelism`` — runner-driven, covers every entry
   in PARALLELISM_CONFIGS (TP / EP / FSDP / PP / VPP and composites).
2. ``test_perf_overhead_is_bounded`` — deterministic-mode ratio bound.
3. ``test_bit_exact_under_racing_streams`` — TP=4 + side-stream contention.
4. ``test_bit_exact_under_jitter`` — TP=4 + cuda._sleep jitter +
   ``NCCL_LAUNCH_RACE_FATAL=1``.
"""
